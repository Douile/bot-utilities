  ...
  if (embed.fields) {
    ...
    for (let i=0;i<embed.fields.length;i++) {
      if ('name' in embed.fields[i]) {
        embed.fields[i].name = exports.truncateString(embed.fileds[i].name, 'fields.name' in overrides ? overrides['fields.name'] : EMBED_LIMITS['fields.name'], '...');
      }
